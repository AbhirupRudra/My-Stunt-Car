...
const int buzPin = 14;      // set digital pin D5 as buzzer pin (use active buzzer)
const int ledPin = 15;      // set digital pin D8 as LED pin (use super bright LED)
const int wifiLedPin = 16;  // set digital pin D0 as indication, the LED turn on if NodeMCU connected to WiFi as STA mode
...
String sta_ssid = "<--your SSID-->";      // set Wifi networks you want to connect to
String sta_password = "<--Your Password-->";  // set password for Wifi networks
...
void ForwardLeft(){
  ...
  digitalWrite(In_4, HIGH);
  analogWrite(En_A, SPEED);
  analogWrite(En_B, SPEED/speed_Coeff);
}
...
void ForwardRight(){
  ...
  digitalWrite(In_3, HIGH);
  digitalWrite(In_4, LOW);
  analogWrite(En_A, SPEED/speed_Coeff);
  analogWrite(En_B, SPEED);
}
